...
canvas.addEventListener('contextmenu', e => { e.preventDefault(); boxStart = null; polygonPoints = []; currentMouse = null; draw(); });
...
function draw() {
  ...
  if (mode === 'polygon' && polygonPoints.length > 0) {
    ctx.strokeStyle = 'blue'; ctx.beginPath(); ctx.moveTo(polygonPoints[0].x, polygonPoints[0].y);
  ...
  if (mode === 'box' && boxStart && currentMouse) {
    ...
    ctx.strokeStyle = 'green'; ctx.setLineDash([6]); ctx.strokeRect(px, py, pw, ph); ctx.setLineDash([]);
  }
}

// --- Toolbar wiring ---
document.getElementById('boxBtn').addEventListener('click', () => { setMode('box'); document.getElementById('boxBtn').classList.add('active'); document.getElementById('polyBtn').classList.remove('active'); });
document.getElementById('polyBtn').addEventListener('click', () => { setMode('polygon'); document.getElementById('polyBtn').classList.add('active'); document.getElementById('boxBtn').classList.remove('active'); });
document.getElementById('exportBtn').addEventListener('click', exportAnnotations);

function setMode(m) { mode = m; boxStart = null; polygonPoints = []; currentMouse = null; document.getElementById('currentMode').textContent = m==='box'?'Box':'Polygon'; draw(); }

function exportAnnotations() { const filename = imageName?`${imageName}.json`:'annotations.json'; const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(annotations, null, 2)); const dl = document.createElement('a'); dl.href = dataStr; dl.download = filename; dl.click(); }